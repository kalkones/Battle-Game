...
function getCurrentUser() { return JSON.parse(localStorage.getItem('rduel_current') || 'null'); }
function saveCurrentUser(user) { localStorage.setItem('rduel_current', JSON.stringify(user)); }
...
const Auth = {
    login: () => {
        ...
        if (user && user.password === pass) {
            AppState.user = user; AppState.isGuest = false; AppState.isLoggedIn = true; saveCurrentUser(AppState.user);
            Auth.onSuccess();
    ...
    register: () => {
        ...
        const usernameKey = isEmail ? id.split('@')[0] + Math.floor(Math.random()*1000) : id;
        const displayUsername = isEmail ? id.split('@')[0] : id;
        const newUser = { username: displayUsername, email: isEmail ? id : '', password: pass, level: 1, icon: 'fa-user', gamesPlayed: 0, bestTime: null };
        const users = getLocalStorageUsers();
        users[usernameKey] = newUser;
        saveLocalStorageUsers(users);
        AppState.user = newUser; AppState.isGuest = false; AppState.isLoggedIn = true; saveCurrentUser(AppState.user);
        Auth.onSuccess();
    },
    loginGuest: () => {
        const randomId = Math.floor(Math.random() * 10000);
        const guestUser = { username: `Guest_${randomId}`, type: 'guest', icon: 'fa-ghost', level: 1, gamesPlayed: 0, bestTime: null };
        AppState.user = guestUser; AppState.isGuest = true; AppState.isLoggedIn = true; saveCurrentUser(AppState.user);
        Auth.onSuccess();
    },
    onSuccess: () => {
    ...
    logout: () => {
        localStorage.removeItem('rduel_current');
...
const UI = {
    updateProfileUI: () => {
        ...
        els.displayLvl.textContent = AppState.isGuest ? "Guest Player" : `Lvl ${AppState.user.level}`;
        els.displayLvl.style.color = AppState.isGuest ? "var(--accent-yellow)" : "var(--accent-cyan)";
...
const Game = {
    ...
    spawnTrash: () => {
        ...
        // --- LOGIKA KESULITAN BERTAHAP (PROGRESSIVE DIFFICULTY) ---
        
        // 1. Hitung jumlah item
        // Ronde 1: 5 item -> Ronde 5: 13 item
        let itemCount = 5 + ((AppState.currentRound - 1) * 2); 
        if(itemCount > 15) itemCount = 15; // Cap maksimal 15 item

        // 2. Hitung durasi (kecepatan)
        // Ronde 1: 3000ms -> Ronde 5: 1400ms
        let duration = 3200 - ((AppState.currentRound - 1) * 450);
        if(duration < 1300) duration = 1300; // Minimal 1.3 detik

        // 3. Hitung peluang bomb
        // Ronde 1: 20% -> Ronde 5: 40%
        let bombChance = 0.20 + (AppState.currentRound * 0.05);

        console.log(`Difficulty: Round ${AppState.currentRound}, Items: ${itemCount}, Time: ${duration}ms, Bomb%: ${(bombChance*100).toFixed(0)}%`);

        for(let i=0; i<itemCount; i++) {
            const typeRand = Math.random();
            let type = 'good';
            let itemDuration = duration + (Math.random() * 400 - 200); // Variasi +/- 200ms
            
            if(typeRand < bombChance) { 
                type = 'bad'; 
                itemDuration += 500; // Bomb sedikit lebih lama
            }
            else if(typeRand > 0.85) { 
                type = 'bonus'; 
                itemDuration = 1200; // Bonus selalu cepat
            }
            
            const top = Math.random() * 60 + 15;
            const left = Math.random() * 70 + 10;
            
            // Pass round number untuk logika gerak
    ...
    createItem: (type, top, left, duration, round) => {
        ...
        // --- LOGIKA GERAK (MOVEMENT) ---
        // Ronde 3 ke atas, item mulai bergerak
        if (round >= 3) {
            const speedFactor = (round - 2) * 0.5; // Semakin tinggi ronde, semakin jauh geraknya
            const moveX = (Math.random() - 0.5) * 80 * speedFactor;
            const moveY = (Math.random() - 0.5) * 40 * speedFactor;
            el.style.setProperty('--dx', `${moveX}px`);
            el.style.setProperty('--dy', `${moveY}px`);
            el.classList.add('moving'); // Class untuk animasi CSS
        }

        el.onpointerdown = (e) => {
            e.preventDefault();
            e.stopPropagation();
            Game.processClick(el, type);
        };
    ...
    showFeedback: (text, color, isPositive) => {
        els.msgSub.innerHTML = text;
        els.msgSub.style.color = color;
        els.msgMain.textContent = isPositive ? "HIT!" : "OUCH!";
        els.msgMain.style.color = color;
    ...
    updateStats: () => {
        ...
        els.statScore.textContent = AppState.currentScore;
    },
    ...
    endGame: () => {
        ...
        if(!AppState.isGuest) {
            ...
            AppState.user.level = Math.floor(AppState.user.gamesPlayed / 2) + 1;
        ...
        }], AppState.isGuest ? 'Guest Match' : 'Ranked');
            ...
            els.histList.innerHTML = sessions.slice(0, 10).map(s => {
                ...
                <div class="history-item">
                    <div>
                        ...
                        <div style="margin-top:5px; font-size:0.9rem;">Mode: <span style="color:${s.mode==='Ranked'?'var(--accent-cyan)':'var(--accent-yellow)'}">${s.mode}</span></div>
                    </div>
